...
TEST_DEP_2_STABLE_URL=https://raw.githubusercontent.com/rolandwalker/button-lock/1feabe411a1eefa061323f54004e8cb02b0e6273/button-lock.el
...
build :
	$(RESOLVED_EMACS) $(EMACS_BATCH) --eval    \
	    "(progn                                \
	      ...
	      (batch-byte-compile))" '$(PACKAGE_NAME)'.el plugins/*.el
...
pkg-version :
	...
	@test '$(PACKAGE_VERSION)' = `perl -0777 -ne 'print "$$1\n" if m{\A\s*\S\s*\S+\s+\S+\s+"(\S+)"}s' '$(PACKAGE_NAME)-pkg.el'` || \
	 (echo "Version numbers out of sync"; false )

dist : not-dirty pkg-version
	@mkdir './$(PACKAGE_NAME)-$(PACKAGE_VERSION)'/
	@cp './plugins/$(PACKAGE_NAME)-'*.el './$(PACKAGE_NAME)-$(PACKAGE_VERSION)/'
	@cp './$(PACKAGE_NAME).el'           './$(PACKAGE_NAME)-$(PACKAGE_VERSION)/'
	@cp './$(PACKAGE_NAME)-pkg.el'       './$(PACKAGE_NAME)-$(PACKAGE_VERSION)/'
...
clean : pkg-version
	...
	@rm -rf './$(PACKAGE_NAME)-$(PACKAGE_VERSION).tar' './$(PACKAGE_NAME)-$(PACKAGE_VERSION)'
	@find . -type l -depth 1 -name '$(PACKAGE_NAME)-*.el' -exec rm '{}' \;
